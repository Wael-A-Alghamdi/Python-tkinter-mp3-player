...
# lets us browse the file system to add audio to playlist
...
# function to remove selected audio from playlist
def remove_from_playlist():
    ...
    try:
        ...
        tkinter.messagebox.showerror("Delete Error", "No audio selected.")
...
def save_playlist():
    global playlist
    if len(playlist) > 0:
        playlist_name = filedialog.asksaveasfilename(title = "Save Playlist", filetypes = (("WZ playlist","*.mldpl"),))
        if playlist_name:
            with open(playlist_name + ".mldpl", "w") as f:
                for list_item in playlist:
                    f.write(list_item + "\n")
    else:
        tkinter.messagebox.showerror("Playlist Save Error", "No audio in playlist, can't save.")

def load_playlist():
    global playlist
    global current_id
    playlist_file = filedialog.askopenfilename(title = "Select Playlist", filetypes = (("WZ playlist","*.mldpl"),))
    if playlist_file:
...
# rewind to the beginning of the audio and play
def rewind():
    global current_id
    if current_id:
        play(current_id)
...
# opens info box
def about():
    about_text = "WZ v0.1 by Wael & Abdulaziz\nBuilt with python, tkinter and pygame."
    tkinter.messagebox.showinfo("About WZ", about_text)

# stop audio and close window
def quit_app():
    global volume
    stop()
...
# add previous audio button
previous_audio_button_image = PhotoImage(file = "img/previous_audio.png")
previous_audio_button = ttk.Button(middle_frame, image = previous_audio_button_image, command = play_previous_audio)
previous_audio_button.grid(row = 0, column = 0)
...
# add next audio button
next_audio_button_image = PhotoImage(file = "img/next_audio.png")
next_audio_button = ttk.Button(middle_frame, image = next_audio_button_image, command = play_next_audio)
next_audio_button.grid(row = 0, column = 3)
...
playlist_frame.grid(row = 0, column = 2, padx = 20)
...
# button for adding audio to playlist
...
# status bar
status_bar = ttk.Label(root, text = "Welcome to WZ v0.1", relief = RIDGE, anchor = W, font = "Arial 10")
